...
~80 curated providers (`CATALOG_VERSION`, now **14** — MoltSets (Bearer) joined on 2026-09-16; Financial Datasets, Exa, Crustdata and Aviato are included, and
...
grant (see [local-run](../architecture/local-run.md)). `CATALOG_VERSION` is now **14**. An `unsupported:true` block is first-class: it tells the analyzer
