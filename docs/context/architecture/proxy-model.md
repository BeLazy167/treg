...
Catalog entries can opt into `strict_query`: `_enforce_catalog_query` rejects bodies, undeclared/duplicate query parameters, missing required inputs and unsupported enum values before credential selection. They can separately opt into `body_allowlist`: `_enforce_catalog_body` rejects undeclared top-level JSON fields and missing required fields. Both apply to catalog calls on every tier, leave unmarked entries unchanged and do not constrain arbitrary raw own-tool relays. Legacy `resource_ownership.requires` accepts a declared body parameter as well as path/query parameters, allowing a POST status utility to authorize an opaque shared-account task id.
