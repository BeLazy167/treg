...
async def test_lusha_is_the_last_rung_of_the_phone_waterfall_and_settles_on_its_own_bill(clients: AsyncClient, enrichment_on, monkeypatch):
    ...
    after Dropleads and the cheaper providers; a miss is free and a matched profile
    ...
    assert plan[-1]["endpoint_id"] == "lusha.people.phone.find" and len(plan) == 8, [c["endpoint_id"] for c in plan]
    def misses():
